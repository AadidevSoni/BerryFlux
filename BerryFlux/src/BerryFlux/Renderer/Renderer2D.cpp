...
struct Renderer2DStorage {
  ...
  static const uint32_t MaxTextureSlots = 16; // TO DO: Render Capabilities
  ...
  // Vertices
  glm::vec4 QuadVertexPositions[4]; // 4 vertices
...
void Renderer2D::Init() {
  ...
  BufferLayout quadVBLayout = {
      ...
      {ShaderDataType::Float, "aTexIndex", true},
      {ShaderDataType::Float, "aTilingFactor", true},
  };
  ...
  Ref<IndexBuffer> quadIB = IndexBuffer::Create(
      quadIndices, s_Data.MaxIndices); // passing as number as it is count of
                                       // indices not size in bytes
...
void Renderer2D::Shutdown() { BF_PROFILE_FUNCTION(); }
...
void Renderer2D::EndScene() {
  ...
  uint32_t dataSize =
      ...
      (uint8_t *)
          s_Data.QuadVertexBufferBase; // we subtract and cast to uint8_t as it
                                       // has one byte so we get the size
                                       // instead of count of elements
...
void Renderer2D::DrawQuad(const glm::vec3 &position, const glm::vec2 &size,
  ...
  // Checking if the max quads are under limit
  ...
  glm::mat4 transform = glm::translate(glm::mat4(1.0f), position) //identity
  matrix that we start off with to position
    * glm::scale(glm::mat4(1.0f), {size.x, size.y, 1.0f});
...
void Renderer2D::DrawQuad(const glm::vec3 &position, const glm::vec2 &size,
  ...
  s_Data.QuadVertexBufferPtr->Color = color;
  s_Data.QuadVertexBufferPtr->TexCoord = {0.0f, 0.0f};
  s_Data.QuadVertexBufferPtr->TexIndex =
      0.0f; // Since we are drawing color, we set it to white
  s_Data.QuadVertexBufferPtr->TilingFactor = 1.0f;
  s_Data.QuadVertexBufferPtr++;
...
void Renderer2D::DrawQuad(const glm::vec3 &position, const glm::vec2 &size,
  ...
  // Getting textureIndex
  float textureIndex = 0.0f;
  for (uint32_t i = 1; i < s_Data.TextureSlotIndex; i++) {
    if (*s_Data.TextureSlots[i].get() ==
        *texture.get()) { // get the pointer and de reference it to compare them
  ...
  if (textureIndex == 0.0f) {
    textureIndex =
        (float)s_Data.TextureSlotIndex; // this is 1 for the first call
  ...
  // OLD WAY
  /*s_Data.TextureShader->SetFloat4(
      "u_Color",
      glm::vec4(1.0f)); // We still have to bind a color or else it will remain
                        // as the previous color used so we make it pure white
                        // which makes no change to the original texture.
  s_Data.TextureShader->SetFloat("u_TilingFactor", tilingFactor);
  texture->Bind();
  ...
  glm::mat4 transform =
      glm::translate(
          glm::mat4(1.0f),
          position) // identity matrix that we start off with to position
      * glm::scale(glm::mat4(1.0f), {size.x, size.y, 1.0f});
  s_Data.TextureShader->SetMat4("u_Transform", transform);
