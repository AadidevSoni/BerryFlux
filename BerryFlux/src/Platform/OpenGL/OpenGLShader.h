...
private:
  ...
  PreProcess(const std::string &source); // GLEnum is the type of shader, string
                                         // is the source code for that shader
  void Compile(const std::unordered_map<GLenum, std::string>
                   &shaderSources); // This will compile the shader source code
                                    // and link it to create a shader program
private:
