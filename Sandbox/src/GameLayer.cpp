...
void GameLayer::OnUpdate(BerryFlux::Timestep ts)
{
  ...
  //Camera follow
	...
	// Render
	BerryFlux::RenderCommand::SetClearColor({ 0.0f, 0.0f, 0.0f, 1 }); //Black background
...
void GameLayer::OnEvent(BerryFlux::Event& e)
{
	...
	dispatcher.Dispatch<WindowResizeEvent>(BF_BIND_EVENT_FN(GameLayer::OnWindowResize));
	dispatcher.Dispatch<MouseButtonPressedEvent>(BF_BIND_EVENT_FN(GameLayer::OnMouseButtonPressed));
}
