...
void Level::CreatePillar(int index, float offset)
{
  ...
  //Random Center
	...
	pillar.TopPosition.y = 10.0f - ((10.0f - center) * 0.2f) + gap * 0.5f; //Position Top Pillar
	pillar.BottomPosition.y = -10.0f - ((-10.0f - center) * 0.2f) - gap * 0.5f; //Position Bottom Pillar
}
...
bool Level::CollisionTest()
{
    ...
    for (auto& p : m_Pillars)
    {
        ...
        bool overlapTop =
            std::abs(playerPos.x - p.TopPosition.x) <
        ...
        bool overlapBottom =
            std::abs(playerPos.x - p.BottomPosition.x) <
