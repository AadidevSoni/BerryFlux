...
{
	m_DownFlapTexture = Texture2D::Create("assets/textures/yellowbird-downflap.png");
	m_ShipTexture = Texture2D::Create("assets/textures/flappyBird.png");
	m_UpFlapTexture = Texture2D::Create("assets/textures/yellowbird-upflap.png");
}

void Player::OnUpdate(BerryFlux::Timestep ts)
{
	m_AnimationTimer += ts;
	m_Time += ts;
...
{
	BerryFlux::Ref<BerryFlux::Texture2D> currentTexture;

	float frameTime = fmod(m_AnimationTimer, 0.3f);

	if (frameTime < 0.1f)
			currentTexture = m_DownFlapTexture;
	else if (frameTime < 0.2f)
			currentTexture = m_ShipTexture;
	else
			currentTexture = m_UpFlapTexture;
	m_ParticleSystem.OnRender();
	Renderer2D::DrawQuad(
			{ m_Position.x, m_Position.y, 0.5f },
			{ 1.0f, 1.3f },
			glm::radians(GetRotation() + 0.0f),
			currentTexture
	);
}
